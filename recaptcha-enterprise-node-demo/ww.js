...
import { isSearchResultsPage, pushToAllDataLayers } from 'js/analytics';
...
class SiteSearch extends Component {
        ...
        return html`
            <ul style=${styleMap({ display: hide ? 'none' : '' })} class="cmp-site-search__autocomplete" @keydown="${this.autocompleteKeydown.bind(this)}">
                ...
                : map(results, (item, index) => html`
                        <li>
                            <a class="${componentName}__autocomplete-item" data-index="${index}" @click="${this.autocompleteClick.bind(this)}" href="${item.link}" @blur="${this.closeAutocomplete.bind(this)}">${map(item.title.toLowerCase().split(term.toLowerCase()), (part, i) => {
                    if (i === 0) {
                        return html`${part}`;
                    }
    ...
    updateResultState(data, isLoading, page, term) {
        ...
        const filterLabelsRecommended = Array.from(this.$els.filters || [])
        .filter((f) => this.activeSecondaryTags.includes(f.dataset?.tagId))
        .map((f) => f.innerText?.trim())
        .filter(Boolean);
        if (
            ...
            && this.pendingRecommendedInPageSubmit
        ) {
            console.log(' 5.1 updateResultState cmp-search-results found');
            this.searchAllDataLayerEvent({
                name: 'search_submit',
                term: this.$els.input.value,
                type: 'recommended',
                numberOfResults: this.pagination?.totalMatches ?? 0,
                filterLabelsRecommended,
            });
            this.pendingRecommendedInPageSubmit = false;
        }
                    ...
                    suggestionEl.addEventListener('click', () => {
                        setTimeout(() => {
                            ...
                            if (isSearchResultsPage()) {
                                this.pendingRecommendedInPageSubmit = true;
                            }
